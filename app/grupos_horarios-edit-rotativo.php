...
$form_data = $gruposHorarios->cargarDatosGrupoHorarioRotativo($grupo_id, $empresa_id);
if (!$form_data) {
    header('Location: grupos_horarios.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $post_data = procesarFormularioRotativo($_POST);
    
    // Validar datos
    $errors = GrupoHorarioValidator::validarHorarioRotativo($post_data);
    
    if (empty($errors)) {
        // Actualizar grupo horario usando la clase centralizada
        $resultado = $gruposHorarios->actualizarGrupoHorarioRotativo($grupo_id, $post_data, $empresa_id);
...
function renderEditGrupoHorarioRotativoContent($form_data, $errors, $empleados, $grupo_id) {
    ob_start();
    ?>
    <!-- Breadcrumb -->
    <?php 
    Breadcrumb::render([
        ...
        ['label' => 'Editar Grupo Horario Rotativo']
    ]); 
    ?>

    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Editar Grupo de Horario Rotativo</h1>
        <p class="text-gray-600 mt-1">Modifique la configuración del grupo de horarios rotativos</p>
    </div>
    ...
    <!-- Form -->
    <?php echo GrupoHorarioRotativoForm::render($form_data, $errors, $empleados, 'edit', $grupo_id); ?>
...
$content = renderEditGrupoHorarioRotativoContent($form_data, $errors, $empleados, $grupo_id);

// Usar el BaseLayout para renderizar la página completa
BaseLayout::render('Editar Grupo Horario Rotativo', $content, $config_empresa, $user_data);
?> 